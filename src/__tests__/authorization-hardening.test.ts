...
describe("authorization hardening", () => {
  it("prevents sponsor UI from approving profiles directly", () => {
    ...
    const parrainagesTabs = source(
      "src/components/sponsorship/parrainages-tabs.tsx",
    );
    const invitationCard = source(
  ...
  it("blocks profile approval without confirmed sponsorship in the database", () => {
    const admissionInvariantMigration = migrationSources().find(
      ({ fileName }) =>
        fileName.includes("enforce_confirmed_sponsor_before_admin_approval"),
    );
  ...
  it("routes sponsor requester profile visibility through a scoped RPC", () => {
    ...
    expect(normalizeSql(parrainagesPage)).toContain(
      'rpc( "get_sponsor_requester_profiles"',
    );
  ...
  it("does not reintroduce a profiles policy that queries sponsorship requests", () => {
    ...
    const durableAndLaterProfilePolicyText = durableAndLaterMigrations
      ...
      .filter((statement) =>
        /\bon\s+(?:public\.)?profiles\b/.test(normalizeSql(statement)),
      )
      .map(normalizeSql)
      .join("\n");
  ...
  it("freezes trusted sponsorship and invitation transition identity fields", () => {
    ...
    expect(migrationText).toContain(
      "private.prevent_sponsorship_request_identity_change",
    );
    expect(migrationText).toContain(
      "NEW.requester_id IS DISTINCT FROM OLD.requester_id",
    );
    expect(migrationText).toContain(
      "NEW.sponsor_id IS DISTINCT FROM OLD.sponsor_id",
    );
    expect(migrationText).toContain(
      "NEW.sponsor_handle IS DISTINCT FROM OLD.sponsor_handle",
    );
    expect(migrationText).toContain(
    ...
    expect(migrationText).toContain(
      "private.prevent_invitation_identity_change",
    );
    expect(migrationText).toContain(
      "NEW.inviter_id IS DISTINCT FROM OLD.inviter_id",
    );
    expect(migrationText).toContain(
      "NEW.invited_x_handle IS DISTINCT FROM OLD.invited_x_handle",
    );
  ...
  it("adds explicit channel read and write permission checks", () => {
    ...
    expect(migrationText).toContain(
      "ADD COLUMN IF NOT EXISTS write_permission",
    );
    expect(migrationText).toContain(
  ...
  it("enforces the US3 canonical launch channel taxonomy", () => {
    ...
    expect(migrationText).toContain(
      "slug IN ('recrutement', 'aide', 'random')",
    );
  });
  ...
  it("stores chat images with private signed media paths", () => {
    ...
    expect(messageInput).toContain(
      "mapMessageRowToMessageWithAuthor(insertedMessage as MessageRow)",
    );
    ...
    expect(migrationText).toContain(
      "private.can_current_user_access_chat_media_path",
    );
    expect(migrationText).toContain(
      "COALESCE(c.write_permission, 'all') = 'all'",
    );
    expect(migrationText).toContain(
  ...
  it("supports admin-only chat message pinning", () => {
    ...
    expect(messageArea).toContain(
      "document.getElementById(getMessageDomId(messageId))",
    );
    expect(messageArea).toContain(
  ...
  it("supports chat message replies with same-channel database guards and pinned-message scrolling", () => {
    ...
    expect(migrationText).toContain(
      "ADD COLUMN IF NOT EXISTS reply_to_message_id",
    );
  ...
  it("only notifies mentions after a successful message insert", () => {
    ...
    const [errorBranch, successBranch] =
      insertElseIndex === -1
        ? [sendResultBlock, ""]
  ...
  it("uses canonical chat slug links for chat mention notifications", () => {
    const messageInput = source("src/components/chat/message-input.tsx");
    const notificationProvider = source(
  ...
  it("blocks the composer UI when the active channel is not writable", () => {
    ...
    expect(chatLayout).toContain(
      "Seuls les admins peuvent publier dans Événements.",
    );
  ...
  it("filters global message search to canonical launch chat channels", () => {
    ...
    expect(chatLayoutPage).toContain(
  ...
  it("keeps destructive admin user actions behind server-side safeguards", () => {
    ...
    const adminButtons = source(
      "src/components/admin/approve-reject-buttons.tsx",
    );

    expect(adminClient).toContain("SUPABASE_SERVICE_ROLE_KEY");
    expect(adminClient).toContain("NEXT_PRIVATE_SUPABASE_SECRET_KEY");
    expect(adminClient).toContain("NEXT_PRIVATE_SUPABASE_PASSWORD");
    ...
    expect(adminButtons).toContain(
      "deleteConfirmation !== deleteConfirmationValue",
    );
