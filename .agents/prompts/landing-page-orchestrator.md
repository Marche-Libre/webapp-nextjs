...
If the repository has a clearly established French route convention for public acquisition pages, and no collision exists, you may use:

```txt
/acces-prive
```

Use exactly one of these routes. Prefer `/early-access` unless the existing route structure strongly indicates that `/acces-prive` is the better local convention.
