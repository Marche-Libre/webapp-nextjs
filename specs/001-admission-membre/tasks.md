...
- [ ] T007 Resolve admission data model decision from `DEC-005`: recommended Beta 1 canonical model is `profiles.status` plus `sponsorship_requests`; `invitations` may remain as member-referral compatibility feeding the same status path.
- [ ] T008 Resolve status terminology: runtime status remains `rejected` unless a migration changes it; product/spec language may call this `refused` only as a documented alias.
...
- [ ] T024 Add or update a focused test/SQL check for the selected sponsorship/invitation model and relevant RLS policies, prioritizing non-admin self-update attempts against `profiles.status`.
