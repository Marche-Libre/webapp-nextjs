...
| Admin dashboard/profile listing | Admin | Admin can view candidate/member status | Admin layout gates route; pages query broad profile rows | `src/app/(app)/admin/layout.tsx`; `src/app/(app)/admin/page.tsx`; `src/app/(app)/admin/utilisateurs/page.tsx` | verified | follow-up story input | Production RLS must support admin reads |
